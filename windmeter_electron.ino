...
if (sensorVoltage <= voltageMin){
 windSpeed = 0; //Check if voltage is below minimum value. If so, set wind speed to zero.
  ...
  windSpeed = (sensorVoltage - voltageMin) / (voltageMax - voltageMin) * windSpeedMax; //For voltages above minimum value, use the linear relationship to calculate wind speed.
  Particle.publish("e_s", String(windSpeed, 1), 60, PRIVATE);
}
